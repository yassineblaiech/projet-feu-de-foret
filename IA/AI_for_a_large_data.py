        ...
        for line in lines[1:]:
            ...
            for i in range(len(data)):
                ...
                if len (list_data)>=400:
                    dict_data[features[i]].append(np.array(list_data).reshape(-1,400))
                else:
...
def create_multi_input_model():
    ...
    main_input = Input(shape=(128, 400, 1), name='main_input') # Assuming channel_last format
    ...
    for feature_name in ['spectral_centroid', 'spectral_bandwidth', 'spectral_rolloff', 'zero_crossing_rate', 'rms_energy']:
        input_layer = Input(shape=(1, 400), name=f'{feature_name}_input')
        additional_inputs.append(input_layer)
        # Example of processing additional inputs (reshape if necessary)
        y = Reshape((400,))(input_layer)  # Reshape for Dense layer
        y = Dense(64, activation='relu')(y)
        x = Concatenate()([x, y])

    # Spectral Contrast has a different shape
    spectral_contrast_input = Input(shape=(7, 400), name='spectral_contrast_input')
    ...
    # MFCCs
    mfccs_input = Input(shape=(13, 400), name='mfccs_input')
